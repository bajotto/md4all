...
/**
 * Realça (e torna clicável, via data-attrs + delegação no host) os tokens de
 * PKM no WYSIWYG: wikilinks `[[nota]]` e tags `#tag`. São apenas decorations —
 * o texto no markdown continua intacto (sem round-trip a manter).
 */
...
function buildDecorations(doc: PMNode): DecorationSet {
  ...
  doc.descendants((node, pos) => {
    ...
    while ((m = TAG_RE.exec(text)) !== null) {
      const tag = m[1]
      const tokenStart = m.index + (m[0].length - (tag.length + 1)) // posição do '#'
