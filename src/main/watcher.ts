...
export function watchVault(vaultId: string): void {
  ...
  watcher
    .on('add', (p) => {
      void touchNote(vaultId, rel(p))
      emit('vault:fs-event', { type: 'add', path: rel(p), vaultId })
    })
    .on('change', (p) => {
      void touchNote(vaultId, rel(p))
      emit('vault:fs-event', { type: 'change', path: rel(p), vaultId })
    })
    .on('unlink', (p) => {
      removeNote(vaultId, rel(p))
      emit('vault:fs-event', { type: 'unlink', path: rel(p), vaultId })
    })
